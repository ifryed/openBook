...
import {
  deleteContentDraftForm,
  type DeleteContentDraftState,
} from "@/app/actions/content-drafts";
import { Link } from "@/i18n/navigation";
import { DraftPublishForm } from "@/components/draft-publish-form";
import { useActionState, useEffect, useId, useRef, useState } from "react";
import { useFormStatus } from "react-dom";
import { useTranslations } from "next-intl";

const initial: DeleteContentDraftState = {};
...
/** Edit, Publish, Delete + title field on one line; hint and errors below. */
export function DraftContentActionsCell({
  draftId,
  draftTitle,
}: {
  draftId: string;
  ...
  const t = useTranslations("Drafts");
  const [state, formAction] = useActionState(deleteContentDraftForm, initial);
  const [deleteArmed, setDeleteArmed] = useState(false);
  const [confirmTitle, setConfirmTitle] = useState("");
  const titleMatches = confirmTitle.trim() === draftTitle.trim();
  const fieldId = useId();
  ...
  return (
    <div className="flex min-w-0 w-full max-w-full flex-col gap-1.5">
      <div className="flex flex-wrap items-start gap-2">
        ...
        <DraftPublishForm draftId={draftId} />
        {!deleteArmed ? (
          <button
            type="button"
          ...
          >
            ...
            <label htmlFor={fieldId} className="sr-only">
              {t("deleteContentDraftConfirmLabel")}
            </label>
            <input
              ref={inputRef}
              id={fieldId}
      ...
      {deleteArmed ? (
        <>
          <p className="text-xs text-muted">{t("deleteContentDraftHint")}</p>
          {confirmTitle.length > 0 && !titleMatches ? (
            <p className="text-xs text-muted">
              {t("deleteDraftBookMustMatch")}{" "}
              <span className="font-medium text-foreground">{draftTitle}</span>
            </p>
          ) : null}
          {state.error ? (
